# Machine Learning Model 2 : GPmodel
...
gp_model <- mgcv::gam(COUNT ~ s(LONGITUDE, LATITUDE, bs = 'gp'), data = dat_cov)
...
dat_cov <- dat_cov %>%
  ...
  dplyr::select(-c(P_CODE, fDEPTH, TRUE_COUNT, TOTAL, SITE_NO, TRANSECT_NO, tier, fGROUP,
                   LONGITUDE, LATITUDE)) %>%
  mutate(across(c( fYEAR,REEF_NAME, REEF_SITE, REEF_SITE_TRANSECT), as_factor)) %>%
  mutate(across(c(CYC, DHW, OT), ~ c(scale(.)))) %>%
  mutate(gp_model = predict(gp_model, newdata = dat_cov))
